...
Descriptores locales: Representaciones que describen caracterısticas
elementales de una vecindad reducida, parte de una region de interes como: forma, color, textura o movimiento. Ejemplo de descriptores: SIFT, SURF. La idea es detectar caracteristicas fuertes en la imagen y luego ser capaz de encontrarlas en otras.
...
- **Detector de esquinas de Harris**: Se computa cada punto (x,y) de la imagen
  ...
  **$c(x, y) = λ1·λ2 − k·(λ1 +λ2)^2 = det(M(x, y))−k· [trace(M(x, y))]^2$**
   ...
   candidatos con una respuesta de menos de una tasa θ de la respuesta maxima
   encontrada.


- **Good Features to Track de Shi-Tomasi**

	Modelo de movimiento de imagen para movimientos afines, que usan para seguir caractersticas.Sugieren usar un modelo de traslacion:
...
En este caso, $A = M$ de Harris. Con el mismo razonamiento , se obtienen los valores $λ_1$ y $λ_2$ de $A$ y se acepta un punto candidato si:
$$
c(x,y) = min(λ_1, λ_2) > θ max_{x,y}(c(x,y))
$$
...
cada vez mas grande, suprimiendo cada vez mas detalles en la imagen.
...
- construır region cuadrada centrada en el punto de interes y orientada
seguun la orientacion seleccionada previamente.
...
- Filtros Haar orientados segun orientacion de la region.
...
template (T) con la imagen fuente (I), deslizandola: Mover el patch un pixel a la vez (de izquierda a derecha y de arriba a abajo). En cada posicion, se calcula una
...
- **Randomized KD-Tress**:
	...
	- Se escoge la dimension de corte aleatoriamente entre las primeras $D$
    dimensiones de mayor varianza.
	- En FLANN utilizan D = 5.
	- Para busqueda, se mantiene una cola de prioridad entre todos los
    arboles para que la busqueda se ordene segun proximidad de los bins.
...
- **Hierarchical k-means Tree**:
	...
	- Detener recursion cuando numero de puntos en region menor que K
...
- **Sparse optical flow**: Manera de especi car de antemano el subconjunto de
	puntos a seguir. La idea es obtener un seguimiento relativamente robusto y confiable.
...
Persistencia Temporal (pequeños movimientos): Caso 1D:

ecuacion optical flow para 1D:
$$
v = -\frac{I_t}{I_x}
$$
...
Para pasar de 1D a 2D se debe cumplir: $I_xu + I_yv + I_t = 0$
con $u$ velocidad en direccion de $x$ y $v$ velocidad en direccion de $y$.
Una ecuacion y dos incognitas no se puede resolver asi, sugerencia, considerar el componente de movimiento que sea perpendicular a la linea descrita por la ecuacion de flujo (problema d eapertura).

#### Problema de apertura

consiste en que una lectura local de la velocidad de un contorno orientado es inherentemente ambigua y dependiendo de las dimensiones de la apertura se percibe el movimiento en distintas direcciones. Es asi como el problema se resuelve considerando mas de un pixel asumiendo que todos los pixeles cercanos se mueven coherentemente. En resumen, se permite resolver el problema de movimiento del pixel central considerando el movimiento de los pixeles vecinos (generando un sistenma de ecuaciones).
Para ello entonces se deben tener buenas caracteristicas a seguir (malas caracteristicas son bajo contraste y bordes).
